...
│                                                  │
│  Document (文档)                                  │
│    projectId / type(prd|persona|...) / status     │
│                                                  │
│  Section (章节)                                   │
│    documentId / title / content(Markdown) / order │
│                                                  │
│  AgentLog (执行日志)                               │
│    projectId / agentName / action / output        │
│                                                  │
└──────────────────────────────────────────────────┘
```

Prisma Client 通过 `DATABASE_URL` 连接 Neon。

### 写入路径（Agent 执行 → DB）

```
LangGraph 节点返回 state 更新
  → agent-runner.ts 检测 update.documents
  → agent-logs.ts upsertDocuments()
  → Prisma: Document.upsert + Section.deleteMany + Section.createMany
  → AgentLog.create (每个节点一条)
  
同时:
  → analyze/route.ts: prisma.project.create (项目创建)
  → feedback/route.ts: (不写 Project，复用已有)
```
...
浏览器访问 /workspace/:id
  ...
  → prisma.project.findUnique({ include: { documents, agentLogs } })
  → 序列化后作为 initialProject props 传入
  → WorkspaceShell 用 initialProject 初始化 state:
      toDocMap(documents) → docs state
      toLogs(agentLogs)   → logs state
  → 渲染 DocNav + DocumentView + AIPanel
```

### 实时更新路径（SSE，不经过 DB 查询）

```
Agent 执行中:
  → agent-runner.ts 把 LangGraph chunk 转成 SSE 事件
  → encodeSSE → controller.enqueue → ReadableStream → HTTP 响应
  → 前端 useEventStream: reader.read() → JSON.parse → onEvent(e)
  → React setState (docs/logs/interrupt) → UI 增量更新
...
| | SSE 实时流 | DB 查询 |
|--|----------|--------|
| 什么时候用 | Agent 正在执行时 | 页面刷新、重新打开 |
| 数据来源 | LangGraph chunk | Prisma 查询 |
| 延迟 | 实时（毫秒级） | 查询级（100-300ms） |
| 作用 | 让用户看到 Agent 正在做什么 | 让用户刷新后恢复现场 |

---

## 与企业级应用的架构差异

面试官可能会问："你这个项目跟公司里真正的产品有什么区别？"

### 主要差异

| 维度 | 本项目 (Demo/面试) | 企业级应用 |
|------|-------------------|-----------|
| **前后端关系** | Next.js 全栈（前后端一个仓库、一个进程） | 前后端分离：React SPA + Go/Java 后端 + API 网关 |
| **Agent 执行** | HTTP 请求内同步执行（SSE 流式响应） | 消息队列异步：Agent 跑在 Worker 里，结果推 WebSocket/回调 |
| **状态持久化** | MemorySaver（单进程内存） | PostgresSaver / Redis + 数据库 checkpoint 表 |
| **用户认证** | 无（单用户 Demo 模式） | OAuth2.0 / JWT / RBAC 权限 |
| **流式传输** | 自定义 SSE 事件协议 | 自研网关（SSE/WebSocket/gRPC stream）+ 中间件链路 |
| **扩展性** | 单实例，无法水平扩展 | K8s 多副本 + 负载均衡 + checkpoint 共享存储 |
| **数据库** | 单库 PostgreSQL（业务+日志） | 读写分离 + Redis 缓存 + ES 日志 + 时序监控 |
| **CI/CD** | git push → Vercel 自动部署 | GitHub Actions → 测试 → Staging → 金丝雀 → 生产 |
| **监控告警** | Vercel 自带基础监控 | Prometheus + Grafana + Sentry + PagerDuty |

### 哪些是故意简化的，哪些是通用的

```
故意简化 (Demo 特化):
  - 全栈 Next.js (而非前后端分离)         ← 一个人开发效率最高
  - 无用户系统                            ← 面试官不需要注册
  - MemorySaver (而非 PostgresSaver)      ← 一条命令跑起来
  - HTTP 同步执行 (而非消息队列异步)        ← Serverless 友好
  - 自定义 SSE (而非自研网关)              ← 50 行代码够用

通用架构 (企业级也这样做):
  - LangGraph StateGraph + Checkpointer    ← Agent 编排的事实标准
  - interrupt/resume HITL                 ← 人机协作的范式
  - SSE 流式推送进度                       ← 长任务必选
  - 文档增量落库 (执行中写 DB)              ← 断线恢复的基础
  - Agent 层与 HTTP 层解耦                ← 可迁移到任何传输层
```

### 面试话术

> "这个项目我是故意用 Next.js 全栈做的——一个人交付，TypeScript 从前写到后效率最高。但 Agent 核心层跟 HTTP 层是完全解耦的——`npx tsx test-invoke.ts` 命令行就能跑完整链路，证明 Agent 不依赖 Next.js。
>
> 企业级会用消息队列把 Agent 执行从 HTTP 请求里解耦出来，前端用 WebSocket 订阅执行进度。PostgresSaver 替代 MemorySaver 做 checkpoint 持久化，K8s 多副本也能共享。但核心的 StateGraph + interrupt HITL + SSE 流式的范式是一样的。
>
> 这个项目展示的是我理解这些概念的深度——我知道什么时候该用什么、每种选择的 tradeoff 是什么。不是搭了个 Demo 就完了。"
